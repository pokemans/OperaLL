...
window.addEventListener('DOMContentLoaded', function() {
	opera.extension.postMessage('chromeLL_tchighlight');
	opera.extension.onmessage = function(event){
	  	if(event.data == "true"){
			opera.extension.postMessage('chromeLL_tchighlightcolor');
		}
...
//alert(window.localStorage.getItem('chromeLL_currenttc'));
//window.onload = 'tchl("#fff000")';
//tchl('#fff000');
function tchl(hlcolor){
if(window.localStorage.getItem('chromeLL_currenttc') == undefined){
	window.localStorage.setItem('chromeLL_currenttc', 'test:234341;fsdef:34234');
}
var ls = window.localStorage.getItem('chromeLL_currenttc');
...
function save(tc, tl, hlc){
	...
	window.localStorage.setItem('chromeLL_currenttc', def + "$COL$" + hlc);
}
function ha(hlc){
